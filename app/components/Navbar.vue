<template>
  <div :class="ui.wrapper" v-bind="attrs">
    <div :class="ui.container">
      <div :class="ui.left">
        <slot name="toggle">
          <UButton v-if="isHeaderDialogOpen !== undefined" :icon="appConfig.ui.icons.menu"
            v-bind="($ui?.button?.secondary as any)" :aria-label="`${isHeaderDialogOpen ? 'Close' : 'Open'} sidebar`"
            class="lg:hidden" @click="isHeaderDialogOpen = !isHeaderDialogOpen" />
        </slot>
        <slot name="left">
...
const { ui, attrs } = useUI('ai.navbar',
  toRef(props, 'ui'),
  config,
  toRef(props, 'class'),
  true
